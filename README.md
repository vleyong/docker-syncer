...
3.  **智能同步**：自动比对镜像摘要 (Digest)，未变动则跳过，节省时间与 GitHub Actions 资源。
4.  **按需触发**：手动输入镜像名，想搬什么搬什么，无需维护复杂的同步列表。
...
4. 输入参数：
    * **原镜像名称**: `mysql:8.0` (必填)
    * **保存名称**: 留空则默认与原名一致 (选填)
    * **目标命名空间**: 留空则使用 Secret 默认值 (选填)
5. 点击绿色 **Run workflow** 按钮。

### 2. 自动批量同步 (NEW)
1. 在仓库根目录创建/修改 `images.txt` 文件。
2. 每行填入一个镜像名称，例如：
   ```
   mysql:8.0
   nginx:alpine
   ...
   ```
3. 提交更改。
4. GitHub Actions 会在 **每月 1号 和 15号** (北京时间 08:00) 自动运行，将列表中所有镜像同步到阿里云。
   * 你也可以在 Actions 页面手动触发 **"批量同步 (Batch Sync)"** 工作流。
...
### Q: 如果镜像重名了怎么办？
**A**: Docker 的机制是 **"覆盖"**。如果目标仓库中已经存在同名同 Tag 的镜像（例如 `mysql:8.0`），新的推送会直接覆盖旧的。这正好实现了**镜像更新**的功能。

### Q: 阿里云能不能同时设置多个命名空间？
**A**: **可以**。
1. 在 Secrets 中配置默认的 `ALIYUN_NAMESPACE`。
2. 在手动运行 **Sync to Aliyun** 时，可以在输入框 **"目标命名空间"** 中填入其他的命名空间。脚本会优先使用你填写的这个值。
